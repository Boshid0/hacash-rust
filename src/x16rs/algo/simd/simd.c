...
#define SIMD_MAJ(x, y, z)   (((x) & (y)) | (((x) | (y)) & (z)))
...
#define DECL_STATE_SMALL_simd
#define READ_STATE_SMALL_simd(sc)
#define WRITE_STATE_SMALL_simd(sc)
#define DECL_STATE_BIG_simd
#define READ_STATE_BIG_simd(sc)
#define WRITE_STATE_BIG_simd(sc)
...
#define READ_STATE_SMALL_simd(sc)   do { \
		A0 = (sc)->state[ 0]; \
		A1 = (sc)->state[ 1]; \
		A2 = (sc)->state[ 2]; \
		A3 = (sc)->state[ 3]; \
...
#define WRITE_STATE_SMALL_simd(sc)   do { \
		(sc)->state[ 0] = A0; \
		(sc)->state[ 1] = A1; \
		(sc)->state[ 2] = A2; \
		(sc)->state[ 3] = A3; \
...
#define READ_STATE_BIG_simd(sc)   do { \
		A0 = (sc)->state[ 0]; \
		A1 = (sc)->state[ 1]; \
		A2 = (sc)->state[ 2]; \
		A3 = (sc)->state[ 3]; \
...
#define WRITE_STATE_BIG_simd(sc)   do { \
		(sc)->state[ 0] = A0; \
		(sc)->state[ 1] = A1; \
		(sc)->state[ 2] = A2; \
		(sc)->state[ 3] = A3; \
...
#define ONE_ROUND_SMALL(ri, isp, p0, p1, p2, p3)   do { \
		STEP_SMALL_(WS_ ## ri ## 0, \
			IF,  p0, p1, XCAT(PP4_, SIMD_M3_0_ ## isp)); \
		STEP_SMALL_(WS_ ## ri ## 1, \
			IF,  p1, p2, XCAT(PP4_, SIMD_M3_1_ ## isp)); \
		STEP_SMALL_(WS_ ## ri ## 2, \
			IF,  p2, p3, XCAT(PP4_, SIMD_M3_2_ ## isp)); \
		STEP_SMALL_(WS_ ## ri ## 3, \
			IF,  p3, p0, XCAT(PP4_, SIMD_M3_3_ ## isp)); \
		STEP_SMALL_(WS_ ## ri ## 4, \
			SIMD_MAJ, p0, p1, XCAT(PP4_, SIMD_M3_4_ ## isp)); \
		STEP_SMALL_(WS_ ## ri ## 5, \
			SIMD_MAJ, p1, p2, XCAT(PP4_, SIMD_M3_5_ ## isp)); \
		STEP_SMALL_(WS_ ## ri ## 6, \
			SIMD_MAJ, p2, p3, XCAT(PP4_, SIMD_M3_6_ ## isp)); \
		STEP_SMALL_(WS_ ## ri ## 7, \
			SIMD_MAJ, p3, p0, XCAT(PP4_, SIMD_M3_7_ ## isp)); \
	} while (0)

#define SIMD_M7_0_0   0_
#define SIMD_M7_1_0   1_
#define SIMD_M7_2_0   2_
#define SIMD_M7_3_0   3_
#define SIMD_M7_4_0   4_
#define SIMD_M7_5_0   5_
#define SIMD_M7_6_0   6_
#define SIMD_M7_7_0   0_

#define SIMD_M7_0_1   1_
#define SIMD_M7_1_1   2_
#define SIMD_M7_2_1   3_
#define SIMD_M7_3_1   4_
#define SIMD_M7_4_1   5_
#define SIMD_M7_5_1   6_
#define SIMD_M7_6_1   0_
#define SIMD_M7_7_1   1_

#define SIMD_M7_0_2   2_
#define SIMD_M7_1_2   3_
#define SIMD_M7_2_2   4_
#define SIMD_M7_3_2   5_
#define SIMD_M7_4_2   6_
#define SIMD_M7_5_2   0_
#define SIMD_M7_6_2   1_
#define SIMD_M7_7_2   2_

#define SIMD_M7_0_3   3_
#define SIMD_M7_1_3   4_
#define SIMD_M7_2_3   5_
#define SIMD_M7_3_3   6_
#define SIMD_M7_4_3   0_
#define SIMD_M7_5_3   1_
#define SIMD_M7_6_3   2_
#define SIMD_M7_7_3   3_

#define STEP_BIG_(w, fun, r, s, pp8b)   STEP_BIG w, fun, r, s, pp8b)

#define ONE_ROUND_BIG(ri, isp, p0, p1, p2, p3)   do { \
		STEP_BIG_(WB_ ## ri ## 0, \
			IF,  p0, p1, XCAT(PP8_, SIMD_M7_0_ ## isp)); \
		STEP_BIG_(WB_ ## ri ## 1, \
			IF,  p1, p2, XCAT(PP8_, SIMD_M7_1_ ## isp)); \
		STEP_BIG_(WB_ ## ri ## 2, \
			IF,  p2, p3, XCAT(PP8_, SIMD_M7_2_ ## isp)); \
		STEP_BIG_(WB_ ## ri ## 3, \
			IF,  p3, p0, XCAT(PP8_, SIMD_M7_3_ ## isp)); \
		STEP_BIG_(WB_ ## ri ## 4, \
			SIMD_MAJ, p0, p1, XCAT(PP8_, SIMD_M7_4_ ## isp)); \
		STEP_BIG_(WB_ ## ri ## 5, \
			SIMD_MAJ, p1, p2, XCAT(PP8_, SIMD_M7_5_ ## isp)); \
		STEP_BIG_(WB_ ## ri ## 6, \
			SIMD_MAJ, p2, p3, XCAT(PP8_, SIMD_M7_6_ ## isp)); \
		STEP_BIG_(WB_ ## ri ## 7, \
			SIMD_MAJ, p3, p0, XCAT(PP8_, SIMD_M7_7_ ## isp)); \
	} while (0)
...
one_round_small(u32 *state, u32 *w, int isp, int p0, int p1, int p2, int p3)
{
	...
	STEP2_SMALL(w[16], w[17], w[18], w[19], SIMD_MAJ, p0, p1, pp4k[isp + 4]);
	STEP2_SMALL(w[20], w[21], w[22], w[23], SIMD_MAJ, p1, p2, pp4k[isp + 5]);
	STEP2_SMALL(w[24], w[25], w[26], w[27], SIMD_MAJ, p2, p3, pp4k[isp + 6]);
	STEP2_SMALL(w[28], w[29], w[30], w[31], SIMD_MAJ, p3, p0, pp4k[isp + 7]);
}

static void
simd_compress_small(sph_simd_small_context *sc, int last)
{
	unsigned char *x;
	s32 q[128];
	int i;
