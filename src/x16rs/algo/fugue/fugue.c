...
#define DECL_STATE_SMALL_fugue
#define READ_STATE_SMALL_fugue(state)
#define WRITE_STATE_SMALL_fugue(state)
#define DECL_STATE_BIG_fugue
#define READ_STATE_BIG_fugue(state)
#define WRITE_STATE_BIG_fugue(state)
...
#define DECL_STATE_BIG_fugue \
	DECL_STATE_SMALL_fugue \
	sph_u32 S30, S31, S32, S33, S34, S35;

#define READ_STATE_SMALL_fugue(state)   do { \
		S00 = (state)->S[ 0]; \
		S01 = (state)->S[ 1]; \
		S02 = (state)->S[ 2]; \
		S03 = (state)->S[ 3]; \
...
#define READ_STATE_BIG_fugue(state)   do { \
		READ_STATE_SMALL_fugue(state); \
		S30 = (state)->S[30]; \
		S31 = (state)->S[31]; \
		S32 = (state)->S[32]; \
		S33 = (state)->S[33]; \
...
#define WRITE_STATE_SMALL_fugue(state)   do { \
		(state)->S[ 0] = S00; \
		(state)->S[ 1] = S01; \
		(state)->S[ 2] = S02; \
		(state)->S[ 3] = S03; \
...
#define WRITE_STATE_BIG_fugue(state)   do { \
		WRITE_STATE_SMALL_fugue(state); \
		(state)->S[30] = S30; \
		(state)->S[31] = S31; \
		(state)->S[32] = S32; \
		(state)->S[33] = S33; \
...
fugue2_core(sph_fugue_context *sc, const void *data, size_t len)
{
	...
	CORE_EXIT
	WRITE_STATE_SMALL_fugue(sc);
}

static void
fugue3_core(sph_fugue_context *sc, const void *data, size_t len)
{
	DECL_STATE_BIG_fugue
	CORE_ENTRY
	READ_STATE_BIG_fugue(sc);
	rshift = sc->round_shift;
	switch (rshift) {
		for (;;) {
			sph_u32 q;
