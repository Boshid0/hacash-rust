...
#define BSG5_0(x)      (SPH_ROTR64(x, 28) ^ SPH_ROTR64(x, 34) ^ SPH_ROTR64(x, 39))
#define BSG5_1(x)      (SPH_ROTR64(x, 14) ^ SPH_ROTR64(x, 18) ^ SPH_ROTR64(x, 41))
#define SSG5_0(x)      (SPH_ROTR64(x, 1) ^ SPH_ROTR64(x, 8) ^ SPH_T64((x) >> 7))
#define SSG5_1(x)      (SPH_ROTR64(x, 19) ^ SPH_ROTR64(x, 61) ^ SPH_T64((x) >> 6))
 ...
 */
static void sha3_round(const unsigned char *data, sph_u64 r[8])
{
...
/* see sph_sha3.h */
void sph_sha512_init(sph_sha512_context *sc)
{
	memcpy(sc->val, H512, sizeof H512);
	sc->count = 0;
}

void sph_sha512_update(sph_sha512_context *sc, const void *data, size_t len)
{
	unsigned current = (unsigned)sc->count & (_BLEN - 1U);
...
static void sha512_addbits_and_close (sph_sha512_context *sc, unsigned ub, unsigned n, void *dst, unsigned rnum)
{
	unsigned current, u;
	current = (unsigned)sc->count & (_BLEN - 1U);
	...
	if (current > _MAXPAD) {
		memset(sc->buf + current, 0, _BLEN - current);
		sha3_round(sc->buf, sc->val);
		memset(sc->buf, 0, _MAXPAD);
	} else {
		memset(sc->buf + current, 0, _MAXPAD - current);
	}

	sph_enc64be_aligned(sc->buf + _MAXPAD, sc->count >> 61);
	sph_enc64be_aligned(sc->buf + _MAXPAD + _WLEN,
...
{
	sha512_addbits_and_close(cc, 0, 0, dst, 8);
	//sph_sha512_init(cc);
}
