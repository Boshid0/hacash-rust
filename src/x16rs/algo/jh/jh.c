...
#define WRITE_STATE_jh(state)   do { \
		(state)->H.wide[ 0] = h0h; \
		(state)->H.wide[ 1] = h0l; \
		(state)->H.wide[ 2] = h1h; \
		(state)->H.wide[ 3] = h1l; \
...
{
	jh_close(cc, 0, 0, dst, 16, JH_IV512);
}
...
{
	jh_close(cc, ub, n, dst, 16, JH_IV512);
}
