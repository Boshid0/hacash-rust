...
#define READ_STATE_shabal(state)   do { \
		A00 = (state)->A[0]; \
		A01 = (state)->A[1]; \
		A02 = (state)->A[2]; \
		A03 = (state)->A[3]; \
...
#define WRITE_STATE_shabal(state)   do { \
		(state)->A[0] = A00; \
		(state)->A[1] = A01; \
		(state)->A[2] = A02; \
		(state)->A[3] = A03; \
...
static const sph_u32 A_init_512[12] __attribute__ ((aligned (32)))= {
	C32(0x20728DFD), C32(0x46C0BD53), C32(0xE782B699), C32(0x55304632),
	C32(0x71B4EF90), C32(0x0EA9E82C), C32(0xDBB930F1), C32(0xFAD06B8B),
	C32(0xBE0CAE40), C32(0x8BD14410), C32(0x76D2ADAC), C32(0x28ACAB7F)
};

static const sph_u32 B_init_512[16] __attribute__ ((aligned (32)))= {
...
static const sph_u32 C_init_512[16] __attribute__ ((aligned (32)))= {
...
static void shabal_init(sph_shabal_context *sc, unsigned size)
{
	memcpy(sc->A, A_init_512, sizeof sc->A);
	memcpy(sc->B, B_init_512, sizeof sc->B);
	memcpy(sc->C, C_init_512, sizeof sc->C);
	sc->Wlow = 1;
	sc->Whigh = 0;
	sc->ptr = 0;
}

static void shabal_core(sph_shabal_context *sc, const unsigned char *data, size_t len)
{
	unsigned char *buf;
	size_t ptr;
	DECL_STATE_shabal
	...
	WRITE_STATE_shabal(sc);
	sc->ptr = ptr;
}

static void
shabal_close(sph_shabal_context *sc, unsigned ub, unsigned n, void *dst, unsigned size_words)
{
	unsigned char *buf;
	size_t ptr;
	int i;
...
//extern shabal_compress(uint8_t *out, uint8_t *in, uint64_t inlen);
